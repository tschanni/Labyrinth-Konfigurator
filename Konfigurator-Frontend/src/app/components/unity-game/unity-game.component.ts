...
@Component({
  ...
  styleUrls: ['./unity-game.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class UnityGameComponent implements OnInit {

  constructor(private unityService: UnityService, private router: ActivatedRoute) { }

  public mapId;

  ngOnDestroy(): void {
    var callback;
    this.unityInstance.Quit(callback)
    console.log("Ends Unitygame: " + callback);
  ...
  async ngOnInit() {
    ...
    this.router.params.subscribe( params => {
      ...
      {
        this.unityService.openGameWithMapID(+params['gameid']);
        this.mapId = +params['gameid'];
      }
    })   
    ...
    createUnityInstance(canvas, config, (progress: any) => {
      ...
      this.unityService.setUnityInstance(this.unityInstance);
      if(this.mobileAndTabletCheck())
      {
        console.log("MOBILE")
        unityInstance.SendMessage('GameManager', 'ShowTouchUI');
      }
      else
      {
        console.log("Desktop")
      ...
      if(message == "DrawMap")
      {
        console.log("DrawMap == " + message);

        this.unityService.UnityLoaded();
      }
